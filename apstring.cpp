...
apstring::apstring(int size, char ch)
{
	myLength = size;
	myCapacity =size+1;
	myCstring = new char[myCapacity];
	memset(myCstring, ch, size);
    myCstring[size] = '\0';           // make c-style string zero length
}
...
void apstring::tolowercase()
{
	int b = 0;
	while (b < myLength)
	{
		myCstring[b] = tolower(myCstring[b]);
		b++;
	}
}

void apstring::touppercase()
{
	int b = 0;
	while (b < myLength)
	{
		myCstring[b] = toupper(myCstring[b]);
		b++;
	}
}
...
{//replaces all occurances of strOld with strNew
	...
	while (b >= 0)
	{
		...
		tmpS += remS.substr(0, b) + strNew;
		remS = remS.substr(b + l, e);
		b = remS.find(strOld);
	}
	(*this) = tmpS + remS;
...
void apstring::parse_string()
{
	int b, e = myLength;
	if (e == 0)	return;

	for (b = 0; b < e; b++)
		if (int(myCstring[b]) < 0) myCstring[b] = '?'; //treatment of wrong characters, 2008 March, //??!!??
	b = 0;
...
//description:		reads a line from input stream
//precondition:		opened input stream
