////////////////////////////////
// This adaptation is based on a free source code:
// Matrix TCL Lite v1.13
// Copyright (c) 1997-2002 Techsoft Pvt. Ltd. 
//
// Functionalities added: 
// GetRowScale(), GetColScale(), GetRow(), GetCol(), modNorm()
//
// Problems corrected: 
// SetSize() - ( original version would crash if resized to smaller number of columns )
//
// Compiler macro commands were adjusted
//
//////////////////////////////
...
// deal with shortcomings of various (mainly old) versions of popular compilers.
...
public:
   ...
   //interfaces to extract vectors as matrices
   matrixT GetRow(size_t row)  _NO_THROW; 
   matrixT GetCol(size_t col)  _NO_THROW;
   ...
   T Norm () _NO_THROW;
   T modNorm (size_t workmode = 0) _NO_THROW;
   ...
   bool GetRowScale(size_t b, size_t e, T &Mn, T &Mx) _NO_THROW;
   bool GetColScale(size_t b, size_t e, T &Mn, T &Mx) _NO_THROW;
...
matrixT::SetSize (size_t row, size_t col) _NO_THROW
{
   ...
   if (row > oldRow)
   {
	   ...
	   oldRow = row;// prevents out-of-range in the copying below
...
// calculate sum of moduluses of a matrix
MAT_TEMPLATE inline T
matrixT::modNorm (size_t workmode) _NO_THROW
//workmode: 0 - default, 1 - only off-diagonal elements, 2 - only diagonal elements
{
	T retVal = T(0);
	...
	for (size_t i=0; i < _m->Row; i++)
      for (size_t j=0; j < _m->Col; j++)
	  {
		 if ((workmode == 1) && (i == j)) continue;
		 if ((workmode == 2) && (i != j)) continue;
         retVal += fabs(_m->Val[i][j]);
	  }
...
MAT_TEMPLATE inline bool
matrixT::GetRowScale(size_t b, size_t e, T &Mn, T &Mx) _NO_THROW
//Calculates min and max values for the range of [b..e] of matrix rows. 
//Returns true if they are normalised.
{
	bool rs = true;
	size_t f, u;
	Mx = Mn = _m->Val[b][0];
	...
	{	
		for (f = 0; f < _m->Col; f++)
		{
			if (_m->Val[u][f] > Mx) Mx = _m->Val[u][f];
			if (_m->Val[u][f] < Mn) Mn = _m->Val[u][f];
		}
		if ( (Mn < 0.0) || (Mx > 1.0) )	rs = false;
	}
	return rs;
}

MAT_TEMPLATE inline bool
matrixT::GetColScale(size_t b, size_t e, T &Mn, T &Mx) _NO_THROW
//Calculates min and max values for the range of [b..e] of matrix columns. 
//Returns true if they are normalised.
{
	bool rs = true;
	size_t f, u;
	...
	{	
		for (f = 0; f < _m->Row; f++)
		{
			if (_m->Val[f][u] > Mx) Mx = _m->Val[f][u];
			if (_m->Val[f][u] < Mn) Mn = _m->Val[f][u];
		}
		if ( (Mn < 0.0) || (Mx > 1.0) )	rs = false;
	}
	return rs;
