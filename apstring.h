...
class apstring
{
  ...
  // constructors/destructor
	...
	apstring(int size);						// construct string with length n
		apstring(int size, char ch);			// construct string with length n filled with character ch
  ...
  // assignment
	...
	const apstring & operator = ( int i );				  // assign number
  ...
  // accessors
	...
	void tolowercase();							//converts string to lower case characters
	void touppercase();							//converts string to upper case characters
	void getline(istream &is);					//reading a line from input stream
	
	void getlinewithtabs(istream &is);		//reading a line without stopping on a tab-symbol
	void parse_string();						//parses string from blanks and control characters
	bool replace(const apstring strOld, const apstring strNew);	//replaces all occurances of strOld with strNew
